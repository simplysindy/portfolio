...
function DesignLabel({ number, title, subtitle }: { number: number; title: string; subtitle: string }) {
  return (
    <div className="border-t-2 border-gray-200 dark:border-gray-700 pt-12 pb-6 px-6 max-w-6xl mx-auto">
      <div className="flex items-baseline gap-5">
        <span className="text-7xl font-black text-gray-200 dark:text-gray-800 select-none leading-none">
          {String(number).padStart(2, "0")}
        </span>
        <div>
          <h2 className="text-2xl font-bold tracking-tight">{title}</h2>
          <p className="text-gray-400 dark:text-gray-500 mt-1 text-sm">{subtitle}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto relative">
        <h2 className={`${syne.className} text-5xl font-extrabold text-gray-900 mb-2 tracking-tight`}>
          Selected Work
        </h2>
        <div className="w-14 h-1.5 bg-gradient-to-r from-[#007cf0] to-[#00dfd8] rounded-full mb-14" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          >
            <div className="bg-white rounded-2xl border border-gray-200/80 overflow-hidden shadow-sm hover:shadow-2xl hover:border-blue-300/60 transition-all duration-300 h-full">
              ...
              <div className="p-7">
                <h3 className={`${syne.className} text-xl font-bold text-gray-900`}>
                  {projects[0].title}
                </h3>
                  ...
                  <div className="flex gap-3 mt-4">
                    {projects[0].links.map((link, i) => (
                      <a
                        key={i}
                        href={link.url}
                        className={`${syne.className} text-sm font-semibold text-[#007cf0] hover:text-[#005bb5] transition`}
                      >
                        {link.label}&nbsp;&rarr;
                      </a>
                    ))}
                ...
                <div className="flex flex-wrap gap-2 mt-5">
                  {projects[0].tags.map((tag, i) => (
                    <span
                      key={i}
                      className="px-3 py-1 text-[11px] font-semibold tracking-wide uppercase bg-blue-50 text-blue-600 rounded-full"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <div className="flex flex-col gap-5">
            {projects.slice(1, 3).map((project, idx) => (
              <motion.div
                ...
                transition={{ type: "spring", stiffness: 260, damping: 20 }}
              >
                <div className="bg-white rounded-2xl border border-gray-200/80 overflow-hidden shadow-sm hover:shadow-xl hover:border-blue-300/60 transition-all duration-300 h-full flex flex-col">
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <h3 className={`${syne.className} text-base font-bold text-gray-900`}>
                      {project.title}
                    </h3>
                    <p className="text-gray-500 mt-1 text-xs leading-relaxed line-clamp-2">
                      {project.description}
                    </p>
                    <div className="flex flex-wrap gap-1.5 mt-auto pt-3">
                      {project.tags.slice(0, 3).map((tag, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 text-[10px] font-semibold tracking-wide uppercase bg-gray-100 text-gray-500 rounded-full"
                        >
                          {tag}
                        </span>
                      ))}
          ...
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 260, damping: 20 }}
          >
            <div className="bg-white rounded-2xl border border-gray-200/80 overflow-hidden shadow-sm hover:shadow-xl hover:border-blue-300/60 transition-all duration-300">
              <div className="flex flex-col sm:flex-row">
                <div className="relative sm:w-2/5 aspect-[16/9] sm:aspect-auto sm:min-h-[220px] overflow-hidden">
                  <Image
                ...
                <div className="p-7 sm:w-3/5 flex flex-col justify-center">
                  <h3 className={`${syne.className} text-xl font-bold text-gray-900`}>
                    {projects[3].title}
                  </h3>
                  <p className="text-gray-500 mt-2 text-sm leading-relaxed">
